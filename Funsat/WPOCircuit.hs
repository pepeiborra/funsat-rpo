...
-}
module Funsat.WPOCircuit
   (
   -- * Circuit language extensions for WPO circuits
   -- ** The language extension for WPO circuits
    WPOCircuit(..), Co, CoWPO, termGt_, termEq_
   -- ** The language extension for multi term WPO comparisons
   ,WPOExtCircuit(..)
   ...
   -- * Type classes for WPO identifiers
...
class HasStatus id    where lexSel_vv     :: id -> [Family.Var id]
                            lexPerm_vv    :: id -> [[Family.Var id]]

class Circuit repr => WPOCircuit repr where
    type CoWPO_ repr (termF :: * -> *) v' v :: Constraint
    termGt, termGe, termEq :: (termF ~ Family.TermF repr
                              ,id    ~ Family.Id termF
                              ,v     ~ Family.Var id
                              ,Foldable termF, HasId1 termF
                              ,Eq (Term termF v')
                              ,HasPrecedence id, HasStatus id
                              ,CoWPO repr termF v' v
                              ) =>
...
type CoWPO repr term tv v = (Co repr v, CoWPO_ repr term tv v)

class (WPOCircuit repr,HasPrecedence id, HasStatus id) =>
      WPOExtCircuit repr id where
...
class WPOCircuit repr => WPOAlgebraCircuit repr id where
  aGt, aEq :: ( id ~ Family.Id termF
              , v ~ Family.Var id
              , termF ~ Family.TermF repr
              , Eq (Term termF v')
              , Foldable termF, HasId1 termF
...
inSel   :: (Circuit repr, HasStatus id, Co repr v, v ~ Family.Var id) => Int -> id -> repr v
inSel i     = input . (!! pred i) . lexSel_vv
lexPerm :: (Circuit repr, HasStatus id, Co repr v, v ~ Family.Var id) => id -> [[repr v]]
lexPerm    = (fmap.fmap) input . lexPerm_vv
...
--          ,Hashable tid, Ord tid
--          ,Hashable tvar, Ord tvar, Show tvar, Pretty tvar)
--          ) => CastWPOCircuit ECircuit.Tree c
--      where
--        castWPOCircuit = castCircuit
...
frozenShared :: CCode -> CMaps term tv v -> FrozenShared term tv v
...
--instance (Hashable id, Ord id, ECircuit c, NatCircuit c, ExistCircuit c, Family.TermF c ~ t) => CastWPOCircuit (Shared id var) c where
--    castWPOCircuit = castCircuit

--instance (ECircuit c, NatCircuit c, ExistCircuit c) => CastWPOCircuit (FrozenShared id var) c id var where
--    castWPOCircuit = castCircuit
...
{-# INLINE liftShared #-}
liftShared :: (CCode -> Lazy.State (CMaps term tv v) CCode) -> Shared term tv v -> Shared term tv v
...
instance ( WPOExtCircuit (Shared (Term termF) tv) (Family.Id termF)
         , WPOAlgebraCircuit (Shared (Term termF) tv) (Family.Id termF)
         , Eq (Family.Id termF)
         ) => WPOCircuit (Shared (Term termF) tv)  where
 type CoWPO_ (Shared (Term termF) tv) termF v' v =
...
foldTree :: (t1 -> b) -> (t1 -> b) -> (TreeF t b -> b) -> Tree t t1 -> b
...
instance (ECircuit c, NatCircuit c, OneCircuit c, WPOCircuit c
         ) =>
  CastCircuit (Tree term) c
 where
  type CastCo (Tree term) c v = ( CoWPO c (TermF term) (Family.Var term) v
...
instance WPOCircuit (Tree term) where
    type CoWPO_ (Tree term) termF tv v = (term ~ Term termF tv)
    termGt = tTermGt
    termEq = tTermEq
...
instance WPOCircuit Eval  where
  type CoWPO_ Eval termF tv v =
      ( Hashable v
      , Eq (Term termF tv)
      , Foldable termF, HasId1 termF
      , Eq(Id termF), HasStatus (Id termF), HasPrecedence (Id termF)
      , Pretty (Id termF), Show (Id termF)
      )

  termGt t u = unEvalM (Right `liftM` (>) evalWPO t u)
  termEq t u = unEvalM (Right `liftM` (~~) evalWPO t u)
...
instance WPOCircuit (WrapEval (Term termF var))  where
   type CoWPO_ (WrapEval (Term termF var)) termF tv v =
       ( var ~ tv
       , Eq var
       , Eq (Term termF var)
       , Foldable termF, HasId1 termF
       , HasStatus (Id termF), HasPrecedence (Id termF)
       , Eq(Id termF), Pretty (Id termF), Show (Id termF)
       , Hashable v
       )
   termGt t u = WrapEval $ unEvalM (Right `liftM` (>)  evalWPO t u)
   termEq t u = WrapEval $ unEvalM (Right `liftM` (~~) evalWPO t u)

instance CastCircuit (WrapEval term) (WrapEval term) where
  type CastCo (WrapEval term) (WrapEval term) v = ()
  castCircuit = id

data WPOEval a v = WPO {(>), (>~), (~~) :: a -> a -> EvalM v Bool}

evalWPO :: forall termF id v var.
           (HasPrecedence id, HasStatus id
           ,Ord v, Hashable v, Show v
...
evalWPO = WPO{..} where
  ...
  s ~~ t
   ...
   | Just id_s <- rootSymbol s
   , Just id_t <- rootSymbol t
   = evalB (precedence id_s `eq` precedence id_t) <&> exeq s t
  ...
  s > t
   ...
   = do
     ...
     anyM (>~ t) (sel 3 af_s tt_s) <|>
       (allM (s >) (sel 4 af_t tt_t)  <&> exgt s t)
   | Just id_t <- rootSymbol t, tt_t <- directSubterms t
   , Just id_s <- rootSymbol s, tt_s <- directSubterms s = do
     af_s <- compFiltering id_s
     af_t <- compFiltering id_t
     anyM (>~ t) (sel 3 af_s tt_s) <|>
       (allM (s >) (sel 4 af_t tt_t)  <&> (evalB (precedence id_s `gt` precedence id_t)
                                         <|>
                                         (evalB (precedence id_s `eq` precedence id_t) <&>
                                          exgt s t)))

   | Just id_s <- rootSymbol s, tt_s <- directSubterms s = do
     ii <- compFiltering id_s
  ...
  exgt s t
   ...
   , Just id_s <- rootSymbol s, ss <- directSubterms s = do
        ...
        ps <- evalPerm id_s
        pt <- evalPerm id_t
        lexD (applyFiltering af_s $ permute ss ps)
             (applyFiltering af_t $ permute tt pt)
  ...
  exeq s t
   ...
   , Just id_s <- rootSymbol s, ss <- directSubterms s = do
        ...
        ps <- evalPerm id_s
        pt <- evalPerm id_t
        lexeqD (applyFiltering af_s $ permute ss ps)
               (applyFiltering af_t $ permute tt pt)
   | otherwise = error "internal error"
...
--  evalPerm :: HasStatus id v => id -> EvalM v (Maybe [Int])
  ...
  compFiltering id = do
    filtering <- mapM (evalB.input) (lexSel_vv id)
    let positions = [ i | (i, True) <- zip [1..] filtering ]
    return positions

  applyFiltering ii tt = selectSafe "WPOCircuit.verifyWPOAF.applyFiltering" (map pred ii) tt
...
{-# INLINE binaryNode #-}
binaryNode :: NodeType v -> EdgeType -> EdgeType -> Graph term1 v -> Graph term2 v -> Graph term v
...
instance Pretty term => WPOCircuit (Graph term)  where
    type CoWPO_ (Graph term) termF tv v = (Pretty (Term termF tv))
    termGt t u = Graph $ do
                   n <- newNode
...
data WPOCircuitProblem term tv v = WPOCircuitProblem
    { rpoProblemCnf     :: CNF
...
toCNF :: (Ord v, Hashable v, Show v, Ord(term tv), Hashable(term tv), Show(term tv)) =>
         FrozenShared term tv v -> WPOCircuitProblem term tv v
...
projectWPOCircuitSolution sol prbl = case sol of
                                    Sat lits   -> projectLits lits
                                    Unsat lits -> projectLits lits
  where
  WPOCircuitProblem _ (FrozenShared _ maps) codeMap bitsMap = prbl
